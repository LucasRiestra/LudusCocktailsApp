...
export const CocktailList: React.FC<CocktailListProps> = ({ cocktails, gridRef, currentPage, setCurrentPage }) => {
  ...
  return (
    <div>
      <div className='cocktail-grid' ref={gridRef}>
        {cocktails.slice((currentPage - 1) * cocktailsPerPage, currentPage * cocktailsPerPage).map(cocktail => {
      ...
      {cocktails.length > 0 && <Pagination totalPages={totalPages} currentPage={currentPage} onPageChange={setCurrentPage} gridRef={gridRef} />}
    </div>
  );
}
